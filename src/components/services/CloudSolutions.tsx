...
export default function CloudSolutions({ data }: CloudSolutionsProps) {
  const features = data?.features && data.features.length > 0 ? data.features : defaultFeatures;
  const subtitle = data?.subtitle || "Future-Ready Infrastructure";
  const heroDescription = data?.heroDescription || "Navigate your cloud journey with confidence. Whether private, public, or hybrid, we design cloud environments that drive agility and reduce operational overhead.";
  const ctaPrimary = data?.ctaPrimaryText || "Start Migration";
